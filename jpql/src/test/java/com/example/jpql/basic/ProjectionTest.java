...
import com.example.jpql.domain.Address;
import com.example.jpql.domain.Member;
import com.example.jpql.domain.Order;
import com.example.jpql.domain.Team;
import com.example.jpql.dto.ResultDto;
...
class ProjectionTest {
    ...
    */
    ...
    void test1() {
        ...
        memberList.forEach(m -> log.info("member 이름 : {}", m.getName()));

        List<Team> teamList = em.createQuery("select m.team from Member m", Team.class).getResultList();
        teamList.forEach(t -> log.info("team 이름 : {}", t.getName()));
    }
    ...
    void test2() {
        ...
        addressList.forEach(a -> log.info("address : {}", a));
    }
    ...
    void test3() {
        ...
        //3. new 명령어로 조회
        List<ResultDto> resultList = em.createQuery("select new com.example.jpql.dto.ResultDto(m.name, m.age) from Member m", ResultDto.class).getResultList();
